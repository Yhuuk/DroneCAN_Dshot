...
uint32_t _dronecan_dshot_DirectionQueryRequest_encode(struct dronecan_dshot_DirectionQueryRequest* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, DRONECAN_DSHOT_DIRECTIONQUERY_REQUEST_MAX_SIZE);
    __dronecan_dshot_DirectionQueryRequest_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > DRONECAN_DSHOT_DIRECTIONQUERY_REQUEST_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (__dronecan_dshot_DirectionQueryRequest_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct dronecan_dshot_DirectionQueryRequest sample_dronecan_dshot_DirectionQueryRequest_msg(void) {
    ...
    struct dronecan_dshot_DirectionQueryRequest msg;
